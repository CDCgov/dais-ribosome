...
//! either SGE or Slurm are used for grid execution.
//!
//! ## Tutorial
//!
...
impl GridTaskInfo {
    ...
    where
        ...
        match (env::var("SGE_TASK_ID"), env::var("SLURM_ARRAY_TASK_ID")) {
            (Ok(sge_task_id), _) => Self::from_sge_env(sge_task_id, args),
            (_, Ok(slurm_array_task_id)) => Self::from_slurm_env(slurm_array_task_id, args),
    ...
    /// Constructs a [`GridTaskInfo`] from SLURM environmental variables.
    ///
    /// ## Errors
    ///
    /// `SLURM_ARRAY_TASK_ID` and `SLURM_ARRAY_TASK_MAX` must be set and be
    /// valid.
    fn from_slurm_env<T>(slurm_array_task_id: String, args: &mut T) -> std::io::Result<Self>
    where
        T: GridCompatibleArgs, {
        let task_id = slurm_array_task_id
            .parse::<usize>()
            .with_context(format!("Invalid SLURM_ARRAY_TASK_ID: {slurm_array_task_id}"))?;

        let task_first = env::var("SLURM_ARRAY_TASK_MIN")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(1);

        let task_last = env::var("SLURM_ARRAY_TASK_MAX").with_context("Invalid SLURM_ARRAY_TASK_MAX")?;
        let task_last = task_last
            .parse::<usize>()
            .with_context(format!("Invalid SLURM_ARRAY_TASK_MAX: {task_last}"))?;
...
impl GridRequestedInfo {
    ...
    /// `-sync yes` in [`Sge`] and `--wait` in [`Slurm`]). The job executes in
    /// the current working directory (`-cwd` in [`Sge`] and default in
    /// [`Slurm`]). The `stdout` and `stderr` stream generated by the scheduler
    /// are merged and written to [`log_path`].
    ///
    /// ## Errors
    ///
    ...
    pub fn submit_job_sync(&self) -> Result<(), SubmitError> {
        ...
        let output_cmd = match self.scheduler {
            ...
            GridScheduler::Slurm => {
                let grid_command = self.command.iter().map(AsRef::as_ref).collect::<OsString>();

                let log_path = (self.get_task_output)(&self.log_path, "%a");

                let mut cmd = Command::new("sbatch");

                cmd.args([
                    "--wait",
                    &format!("--array=1-{task_count}", task_count = self.task_count),
                    "--output",
                ])
                .arg(log_path)
                .arg("--wrap")
                ...
                cmd.output()
                    .with_context("An error using sbatch occurred")
                    .map_err(|e| SubmitErrorRepr::Io(e.into()))?
            }
        };
...
/// Grid array-task environment information parsed from SGE or Slurm.
#[derive(Clone, Debug)]
